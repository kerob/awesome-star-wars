...
class ShoppingList extends Component {
  ...
  handleCompletedToggle(e) {
    ...
    const newGroceryItemsState = [...this.state.groceryItems]; //copies the current state of groceryItems
    //newGroceryItemsState[index].completed = target.checked; //modifies grocery items to toggle checked items
    newGroceryItemsState[index] = {
      //modified code that creates a new object and adds to new grocery items instead of mutating a copy of grocery items
      ...newGroceryItemsState[index],
      completed: target.checked,
    };

    this.setState({
      //uses the copied state variable to replace the existing state
  ...
  handleDelete(e) {
    ...
    newGroceryItemsState.splice(index, 1); ///targets item from copied state and deletes 1 item from index
    ...
    if (isFormValid) {
      ...
      this.setState((state) => {
        return {
          ...
          groceryItems: [...state.groceryItems, newGroceryItemObject], //sets state to a copy of current state plus the new item
          newItemName: "",
        };
      });
    }
