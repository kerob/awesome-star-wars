...
class FormValidation extends Component {
  ...
  handleOnChange(e) {
    ...
    const value = target.type === "text" ? target.value : target.checked; //checks between text type or not text type which on the form is checkbox // !this.state.acceptedTerms
  ...
  validateFields() {
    ...
    this.setState({
      //sets error message if any of previous fields are true
      validationErrors: errors,
    });

    return Object.keys(errors).length === 0; //Checks that number of errors is 0 and submits if true
  }

  render() {
    const {
      //destructuring to replace repetative this.state.validation.item statements
